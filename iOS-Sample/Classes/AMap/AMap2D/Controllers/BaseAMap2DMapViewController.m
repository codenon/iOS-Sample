...
@implementation BaseAMap2DMapViewController
...
- (void)AMapSearchRequest:(id)request didFailWithError:(NSError *)error {
  NSLog(@"%s: searchRequest = %@, errInfo= %@", __func__, [request class],
        error);
}
...
- (NSString *)getApplicationName {
  ...
  return [bundleInfo valueForKey:@"CFBundleDisplayName"]
             ?: [bundleInfo valueForKey:@"CFBundleName"];
}
...
- (void)viewDidAppear:(BOOL)animated {
  ...
  if (_isFirstAppear) {
    self.mapView.visibleMapRect =
        MAMapRectMake(220880104, 101476980, 272496, 466656);
    _isFirstAppear = NO;
