...
@interface NONBPushController ()

@end

@implementation NONBPushController
...
- (IBAction)channelId:(id)sender {
  [self addLogString:[@"Channel ID : "
                         stringByAppendingString:[BPush getChannelId]]];
}

- (BOOL)checkTagString:(NSString *)tagStr {
  NSString *str =
      [tagStr stringByReplacingOccurrencesOfString:@"," withString:@""];
...
- (void)addLogString:(NSString *)logStr {
  ...
  if (preLogString) {
    [self.outputTextView
        setText:[additionStr stringByAppendingString:preLogString]];
  } else {
...
- (NSString *)replaceUnicode:(NSString *)unicodeStr {
  NSString *tempStr1 = [unicodeStr stringByReplacingOccurrencesOfString:@"\\u"
                                                             withString:@"\\U"];
  NSString *tempStr2 =
